...
class Sales extends model
{
    public function getList($offset, $id_company)
    {
        ...
        $sql = $this->db->prepare("SELECT
         sales.id, sales.date_sale, sales.total_price, sales.status, clients.name
         FROM sales LEFT JOIN clients ON clients.id = sales.id_client WHERE sales.id_company = :id_company ORDER BY sales.date_sale DESC LIMIT $offset, 10");
    ...
    public function addSale($id_company, $client_id, $id_user, $quant, $status) {
        ...
        $sql->bindValue(':id_company', $id_company);
        $sql->bindValue(':id_client', $client_id);
    ...
    public function getInfo($id, $id_company) {
        ...
        //$sql = $this->db->prepare("SELECT c.name as client_name, s.date_sale, s.total_price FROM clients c, sales s WHERE c.id = s.id_client AND s.id = :id AND s.id_company = :id_company");
        $sql->bindValue(':id', $id);
        $sql->bindValue(':id_company', $id_company);
        $sql->execute();
        ...
        $sql = $this->db->prepare("SELECT sp.quant, sp.sale_price, i.name FROM sales_products sp, inventory i WHERE i.id = sp.id_product AND sp.id_sale = :id_sale AND sp.id_company = :id_company");
        /*
        $sql = $this->db->prepare("SELECT
                                     sales_products.quant, sales_products.price, inventory.name 
                                  FROM sales_products
                                         LEFT JOIN inventory 
                                         ON inventory.id = sales_products.id_product 
                                         WHERE  
                                         sales_products.id_sale = :id_sale AND sales_products.id_company = :id_company");
        */
    ...
    public function getSalesFiltered($client_name, $period1, $period2, $status, $order, $id_company) 
    {
        ...
        $sql = "SELECT
        clients.name, sales.date_sale, sales.status, sales.total_price
        FROM sales 
    ...
    public function getSoldProducts($period1, $period2, $id_company) {
        ...
        $sql = "SELECT COUNT(*) AS total FROM sales_products AS sp INNER JOIN sales AS s ON sp.id_sale = s.id WHERE sp.id_company = :id_company AND s.date_sale BETWEEN :period1 AND :period2";
        $sql = $this->db->prepare($sql);
        $sql->bindValue(':id_company', $id_company);
        $sql->bindValue(':period1', $period1);
        $sql->bindValue(':period2', $period2);
    ...
    public function getRevenueList($period1, $period2, $id_company) {
        ...
        if($sql->rowCount() > 0) {
            ...
            foreach($rows as $item) {
                $array[$item['date_sale']] = $item['total'];
            }
        }
    ...
    public function getExpensesList($period1, $period2, $id_company) {
        ...
        if($sql->rowCount() > 0) {
            ...
            foreach($rows as $item) {
                $array[$item['date_purchase']] = $item['total'];
            }
        }
    ...
    public function getQuantStausList($period1, $period2, $id_company) {
