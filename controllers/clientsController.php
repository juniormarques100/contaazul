...
class clientsController extends controller {
    ...
    public function index() {
        ...
        if($u->hasPermission('clients_view')) {
            ...
            $data['page_count'] = ceil($data['clients_count'] / $page);
    ...
    public function edit($id) {
        ...
        if($u->hasPermission('clients_edit')) {
            ...
            if(isset($_POST['name']) && !empty($_POST['name'])) {
                ...
                $cli->edit($u->getCompany(), $name, $email, $phone, $stars, $internal_obs, $address_zipcode, $address, $address_number, $address2, $address_neighb, $address_city, $address_state, $address_country, $id);
                header('Location: '.BASE_URL.'/clients');
            }
            $data['clients_info'] = $cli->getInfo($id, $u->getCompany());
    ...
    {
        echo "<h2>Falta fazer esse método!</h2>";
        /*Informações devem serem repetidas nos controllers*/
        $u = new Users();
        $u->setLogged();
        $company = new Companies($u->getCompany());
